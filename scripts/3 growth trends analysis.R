...
growth.gg <- growth.sel.df %>%
  ...
  facet_wrap(~plot_cluster + layer, ncol = 1) +
  gstyle +
...
ntreesclustevent <- event.dbh.df %>%
  group_by(plot_cluster, layer, event_n, species) %>%
  count() %>%
  spread(species, n)
  
#- remove the Synchronized    Midstorey     3                5            1
dbh_sign.df <- event.dbh.df %>%
  group_by(plot_cluster, layer, event_n) %>%
...
quartz(width = 6.3, height = 8, pointsize = 12)
...
# only gap quartz(width = 4, height = 8, pointsize = 12); growth.gg